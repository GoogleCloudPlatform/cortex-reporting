...
FROM 
  `{{ project_id_src }}.{{ dataset_cdc_processed }}.vbpa` VBPA
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.kna1` KNA1
  ON
...
AGGVBPAHEADER AS(
  ...
  FROM 
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.vbpa` VBPA
  INNER JOIN 
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.kna1` KNA1
    ON
      VBPA.mandt=KNA1.mandt
      AND VBPA.kunnr=KNA1.kunnr
  ...
  CAST(AGGPRCD_ELEMENTS.KNUMV AS INT64) KNUMV,
  CAST(AGGPRCD_ELEMENTS.KPOSN AS INT64) KPOSN,
...
FROM 
  `{{ project_id_src }}.{{ dataset_cdc_processed }}.vbak` AS VBAK
INNER JOIN 
  `{{ project_id_src }}.{{ dataset_cdc_processed }}.vbap` AS VBAP 
  ON 
    VBAK.VBELN = VBAP.VBELN 
    AND VBAK.MANDT=VBAP.MANDT
LEFT OUTER JOIN AGGVBEP
  ON 
    VBAP.VBELN = AGGVBEP.VBELN AND
    VBAP.POSNR = AGGVBEP.POSNR 
