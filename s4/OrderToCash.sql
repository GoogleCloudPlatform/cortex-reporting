...
FROM
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.SalesOrders` AS salesorders
LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.Deliveries` AS delivery
  ON
    salesorders.SalesDocument_VBELN = delivery.SalesOrderNumber_VGBEL
    AND salesorders.Item_POSNR = delivery.SalesOrderItem_VGPOS
    AND salesorders.Client_MANDT = delivery.Client_MANDT
LEFT JOIN
...
LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.CustomersMD` AS customers
  ON
    salesorders.SoldtoParty_KUNNR = customers.CustomerNumber_KUNNR
    AND salesorders.Client_MANDT = customers.Client_MANDT
LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.MaterialsMD` AS materials
  ON
    salesorders.MaterialNumber_MATNR = materials.MaterialNumber_MATNR
    AND salesorders.Client_MANDT = materials.Client_MANDT
LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.SalesOrganizationsMD` AS salesorganizations
  ON
    salesorders.Client_MANDT = salesorganizations.Client_MANDT
    AND salesorders.SalesOrganization_VKORG = salesorganizations.SalesOrg_VKORG

LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.DistributionChannelsMD` AS distributionchannel
  ON
    salesorders.Client_MANDT = distributionchannel.Client_MANDT
    AND salesorders.DistributionChannel_VTWEG = distributionchannel.DistributionChannel_VTWEG

LEFT JOIN
  `{{ project_id_src }}.{{ dataset_reporting_tgt }}.RegionsMD` AS region
  ON
    salesorders.Client_MANDT = region.Client_MANDT
    AND customers.CountryKey_LAND1 = region.CountryKey_LAND1
