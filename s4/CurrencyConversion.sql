...
FROM
  `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.currency_conversion` AS CurrencyConversion
...
--   ConvDate
-- FROM
--   `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.currency_conversion`,
--   UNNEST(GENERATE_DATE_ARRAY(
--       (SELECT DATE_ADD(MAX(conv_date), INTERVAL 1 DAY)
--         FROM `{{ project_id_src }}.{{ dataset_cdc_processed_s4 }}.currency_conversion`),
