...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.AddressesMD`
OPTIONS(
...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.adrc` AS ADRC
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.adr6` AS ADR6 ON adrc.client = adr6.client AND adrc.addrnumber = adr6.addrnumber
