...
CREATE OR REPLACE VIEW `{{ project_id_tgt }}.{{ dataset_reporting_tgt }}.CostCentersMD`
OPTIONS(
  description = "Cost Centers Master Data"
)
AS
SELECT
...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.csks` AS csks
INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.cskt` AS cskt ON csks.mandt = cskt.mandt AND csks.kokrs = cskt.kokrs
