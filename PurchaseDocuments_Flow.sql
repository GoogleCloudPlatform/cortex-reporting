...
EKKN AS (
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekkn` AS EKKN
  INNER JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekpo` AS EKPO
...
GoodsReceiptStaging AS (
  ...
  FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekbe`
...
InvoiceReceiptStaging AS (
  ...
  FROM
    `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekbe`
  WHERE
...
FROM `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekko` AS EKKO
LEFT JOIN `{{ project_id_src }}.{{ dataset_cdc_processed }}.ekpo` AS EKPO
  ON
